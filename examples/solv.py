...
task_params = {'target_name' : 'expt',
               'data_file'   : 'consol.pickle2'}
N_train = 100
N_val   = 3
N_test  = 3
...
def main():
    ...
    traindata, valdata, testdata = load_pickle_comp(
        ...
        input_name='key', target_name2='BGB+', target_name1=task_params['target_name'])
    ...
    return [test_loss_morgan, test_loss_neural]
...
if __name__ == '__main__':
    ...
    loss = np.matrix(loss)
    print "Morgan: ", np.mean(loss[:,0]), "+-", np.std(loss[:,0]), "RMSE: ", np.sqrt(np.mean(np.power(loss[:,0], 2)))
    print "Neural: ", np.mean(loss[:,1]), "+-", np.std(loss[:,1]), "RMSE: ", np.sqrt(np.mean(np.power(loss[:,1], 2)))
